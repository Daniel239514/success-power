...
export default function DeleteMasterclassButton({ id }: { id: string }) {
  ...
  async function handleDelete() {
    setBusy(true)
    const result = await deleteMasterclass(id)
    if (result?.error) {
      setError(result.error)
      setBusy(false)
    }
    // On success, deleteMasterclass redirects — so we never reach here.
  }
